import React from 'react';
import { Dialog, DialogHeader, DialogTitle, Button, Badge } from '../ui';
import { 
  Globe, 
  CheckCircle, 
  Clock, 
  HelpCircle, 
  GitBranch, 
  Repeat, 
  Terminal, 
  FileText, 
  Database,
  Link,
  Plus
} from 'lucide-react';

interface StepTypeOption {
  type: string;
  name: string;
  description: string;
  icon: React.ReactNode;
  colorClass: string;
}

interface StepTypeSelectorProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (type: string, isGlobalRef?: boolean) => void;
}

export const StepTypeSelector: React.FC<StepTypeSelectorProps> = ({ 
  isOpen, 
  onClose, 
  onSelect 
}) => {
  const options: StepTypeOption[] = [
    {
      type: 'HTTP_REQUEST',
      name: 'HTTP Request',
      description: 'Make an API call (GET, POST, PUT, DELETE, etc.)',
      icon: <Globe className="h-6 w-6 text-cyan-400" />,
      colorClass: 'bg-cyan-500/10 hover:bg-cyan-500/20 border-cyan-500/30'
    },
    {
      type: 'ASSERTION',
      name: 'Validation Assertion',
      description: 'Validate response fields, status codes, or parameters',
      icon: <CheckCircle className="h-6 w-6 text-emerald-400" />,
      colorClass: 'bg-emerald-500/10 hover:bg-emerald-500/20 border-emerald-500/30'
    },
    {
      type: 'SET_VARIABLE',
      name: 'Extract Variable',
      description: 'Extract body/header values to save as variables',
      icon: <HelpCircle className="h-6 w-6 text-pink-400" />,
      colorClass: 'bg-pink-500/10 hover:bg-pink-500/20 border-pink-500/30'
    },
    {
      type: 'DELAY',
      name: 'Delay/Pause',
      description: 'Pause the execution flow for a specific duration',
      icon: <Clock className="h-6 w-6 text-yellow-400" />,
      colorClass: 'bg-yellow-500/10 hover:bg-yellow-500/20 border-yellow-500/30'
    },
    {
      type: 'CONDITIONAL',
      name: 'Conditional Branch',
      description: 'Execute subsequent steps based on a logical expression',
      icon: <GitBranch className="h-6 w-6 text-indigo-400" />,
      colorClass: 'bg-indigo-500/10 hover:bg-indigo-500/20 border-indigo-500/30'
    },
    {
      type: 'LOOP',
      name: 'Loop iteration',
      description: 'Repeat a set of steps N times or over an array list',
      icon: <Repeat className="h-6 w-6 text-purple-400" />,
      colorClass: 'bg-purple-500/10 hover:bg-purple-500/20 border-purple-500/30'
    },
    {
      type: 'SCRIPT',
      name: 'Custom Script',
      description: 'Evaluate a javascript script/expression context',
      icon: <Terminal className="h-6 w-6 text-teal-400" />,
      colorClass: 'bg-teal-500/10 hover:bg-teal-500/20 border-teal-500/30'
    },
    {
      type: 'DATABASE_QUERY',
      name: 'Database Query',
      description: 'Execute queries against target JDBC connections',
      icon: <Database className="h-6 w-6 text-blue-400" />,
      colorClass: 'bg-blue-500/10 hover:bg-blue-500/20 border-blue-500/30'
    },
    {
      type: 'LOG',
      name: 'Log Message',
      description: 'Output a customized log message in the execution console',
      icon: <FileText className="h-6 w-6 text-gray-400" />,
      colorClass: 'bg-gray-500/10 hover:bg-gray-500/20 border-gray-500/30'
    },
  ];

  return (
    <Dialog isOpen={isOpen} onClose={onClose}>
      <DialogHeader>
        <DialogTitle>Add Test Step</DialogTitle>
      </DialogHeader>
      <div className="p-6 overflow-y-auto max-h-[450px]">
        <p className="text-xs text-muted-foreground mb-4">Choose step type to insert into your visual builder sequence:</p>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {options.map((opt) => (
            <button
              key={opt.type}
              onClick={() => onSelect(opt.type, false)}
              className={`flex items-start space-x-3 p-3 rounded-lg border text-left cursor-pointer transition-all duration-200 hover:scale-[1.01] ${opt.colorClass}`}
            >
              <div className="shrink-0 p-1 bg-secondary rounded">{opt.icon}</div>
              <div className="min-w-0">
                <h4 className="text-sm font-semibold text-foreground">{opt.name}</h4>
                <p className="text-[11px] text-muted-foreground line-clamp-2 mt-0.5">{opt.description}</p>
              </div>
            </button>
          ))}
          
          {/* Global step reference trigger */}
          <button
            onClick={() => onSelect('GLOBAL_REF', true)}
            className="flex items-start space-x-3 p-3 rounded-lg border text-left cursor-pointer transition-all duration-200 hover:scale-[1.01] bg-amber-500/10 hover:bg-amber-500/20 border-amber-500/30 sm:col-span-2"
          >
            <div className="shrink-0 p-1 bg-secondary rounded">
              <Link className="h-6 w-6 text-amber-400" />
            </div>
            <div>
              <h4 className="text-sm font-semibold text-foreground flex items-center">
                <span>Use Reusable Global Step Template</span>
                <Badge variant="secondary" className="ml-2 text-[9px] py-0 px-1 font-bold">reusable</Badge>
              </h4>
              <p className="text-[11px] text-muted-foreground mt-0.5">Reference an admin-managed global step configuration (e.g. login health ping)</p>
            </div>
          </button>
        </div>
      </div>
      <div className="border-t border-border/40 p-4 bg-secondary/15 flex justify-end">
        <Button variant="outline" onClick={onClose}>Cancel</Button>
      </div>
    </Dialog>
  );
};
export default StepTypeSelector;
